...
function yomikomi(img){
    const url=document.getElementById("url").value.replace(/#top/,"");
    $.ajax({
            url: url+".js",
            type: 'GET',
            dataType: 'jsonp',
        ...
        .done(function(jsondata){
            let writeString=characterMake(jsondata,img,url,false);
            let writeString2=[];
            let writeString3= jsondata.phy_bui_name.filter(e=>e!="").length>0 ? characterMake(jsondata,img,url,true) : ""
            const kizyucheck=document.getElementById("kizyu").checked;
            if(jsondata.horse_name&&kizyucheck) writeString2=horsemake(jsondata);
            download(writeString,writeString2,writeString3,jsondata);
        })
}

//キャラコマ作成
//isHenshin=trueのとき変身時のキャラコマを作る
//シザースコーピオンのような変身時のみ存在する場合にはisHenshin=falseは作成しない
function characterMake(jsondata,img,url,isHenshin){
    ...
    if(isHenshin){
        ...
        if(autoHenshin){
            ...
            const buiName=autoBui[jsondata.shuzoku_name];
            writeString+=`        <data name="${buiName}">\n          <data name="HP(${buiName})" type="numberResource" currentValue="${hp}">${hp}</data>\n          <data name="MP(${buiName})" type="numberResource" currentValue="${mp}">${mp}</data>\n          <data name="防護点(${buiName})" type="numberResource" currentValue="${bougo}">${bougo}</data>\n        </data>\n`;
        }
        for(let i=0;i<jsondata.phy_bui_attack.length;i++){
            const hp=jsondata.phy_bui_hp[i];
            const mp=jsondata.phy_bui_mp[i];
            const bougo=jsondata.phy_bui_bougo[i];
            const buiName=jsondata.phy_bui_name[i];
            writeString+=`        <data name="${buiName}">\n          <data name="HP(${buiName})" type="numberResource" currentValue="${hp}">${hp}</data>\n          <data name="MP(${buiName})" type="numberResource" currentValue="${mp}">${mp}</data>\n          <data name="防護点(${buiName})" type="numberResource" currentValue="${bougo}">${bougo}</data>\n        </data>\n`;
        }
    }else{
        const hp=jsondata.HP;
    ...
    for(let i=1;i<=ginou_list.length;i++){
        let num="V_GLv"+i
    ...
    const figical_master_sensei=Number(jsondata.V_GLv20)>0 && jsondata.MS_name.includes("魔将の慧眼") ? true : false;
    const figical_master_mamotiki=Number(jsondata.V_GLv20)>0 && jsondata.MS_name.includes("魔将の経験") ? true : false;
    if(Number(jsondata.V_GLv10)>0||Number(jsondata.V_GLv12)>0||Number(jsondata.V_GLv16)>0||Number(jsondata.V_GLv18)>0||figical_master_mamotiki||figical_master_sensei) writeString+=`\n//-----戦闘準備\n`;
    //チャットパレット(先制判定)
    ...
    if(figical_master_sensei) writeString+=`2d+{フィジカルマスター}+({敏捷度}/6)+${jsondata.sensei_mod||0}+{行動判定}+{行為判定} 【先制判定】\n`;
    //チャットパレット(魔物知識判定)
    ...
    if(figical_master_mamotiki) writeString+=`2d+{フィジカルマスター}+({知力}/6)+${jsondata.mamono_chishiki_mod||0}+{行動判定}+{行為判定} 【魔物知識判定】\n`;
    ...
    else if(autoHenshin) writeString+=`2d+{フィジカルマスター}+({敏捷度}/6)+{回避}+${jsondata.armor_kaihi||0}+${jsondata.shield_kaihi||0}+${jsondata.shield2_kaihi||0}+${jsondata.bougu_kaihi_tokugi||0}+${jsondata.bougu_kaihi_mod||0}+{行動判定}+{行為判定} 【回避力判定】\n`;
    ...
    if(!autoHenshin && isHenshin && Number(jsondata.V_GLv20)>0) writeString+=`2d+{フィジカルマスター}+({敏捷度}/6)+{回避}+${jsondata.armor_kaihi||0}+${jsondata.shield_kaihi||0}+${jsondata.shield2_kaihi||0}+${jsondata.bougu_kaihi_tokugi||0}+${jsondata.bougu_kaihi_mod||0}+{行動判定}+{行為判定} 【回避力判定(その他部位のみ)】\n`;
    ...
    //チャットパレット(魔法)
    const magic_numlist=[5,6,7,8,9,17,24,27];
    const magicName_list=["真語魔法","操霊魔法","神聖魔法","妖精魔法","魔動機術","召異魔法","森羅魔法","奈落魔法"];
    ...
    const magic_list={
        ...
        27:[
            {level:1,iryoku:"k0",name:"スピリット・ナイフ",c:10,mp:3},
            {level:2,iryoku:"k0",name:"ヒーリング・イメージ",c:13,mp:2},
            {level:3,iryoku:"k10",name:"ペシミズム",c:10,mp:4},
            {level:6,iryoku:"k20",name:"リフレッシュ・イメージ",c:13,mp:4},
            {level:7,iryoku:"k30",name:"アビス・ヴォーテックス",c:10,mp:10},
            {level:9,iryoku:"k20",name:"ミアズマ・グレネード",c:10,mp:9},
            {level:10,iryoku:"k40",name:"イモータル・イメージ",c:13,mp:14},
            {level:11,iryoku:"k40",name:"スラッシュ・イメージ",c:10,mp:9},
            {level:13,iryoku:"k50",name:"アビス・ストーム",c:10,mp:18},
        ],
        wizard:[
...
function download(writeString,writeString2,writeString3,jsondata){
    let zip = new JSZip();
    let date=new Date();
    const empty=""
    const image=document.getElementById("image").files[0]
    if(writeString!="") zip.file(`data0.xml`, writeString);
    let i=0
    for(i=0;i<writeString2.length;i++) zip.file(`data${i+1}.xml`, writeString2[i]);
    zip.file(`data${i+1}.xml`, writeString3);
...
function includes(array,word){
    ...
    array.forEach(element => {
        if(element.match(word)) flg=true;
    });
    return flg;
}